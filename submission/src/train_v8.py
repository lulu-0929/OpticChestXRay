"""
光计算加速医学影像诊断 — V8 终极优化训练脚本
==========================================
V8 核心改进：
1. Focal Loss + 类别加权 → 强制关注难分的 NORMAL 样本
2. FC 参数精简 (8192->256->64->2) → 防过拟合
3. 验证集分层(NORMAL 45-50%)采样 → NORMAL 占比不低于 35%，消除验证集偏差
4. NORMAL 类别过采样 3x + 数据增强翻倍
5. 早停耐心值 25 epoch + 更激进 LR 衰减
6. TTA 增强投票数提升至 10
"""
...
epochs = 60
lr = 1e-3
weight_decay = 5e-4           # 加强正则化
dropout_rate = 0.5            # 保持 Dropout
focal_gamma = 3.0             # Focal Loss gamma 参数
TTA_NUM = 15                  # TTA 增强投票数
NORMAL_OVERSAMPLE = 5         # NORMAL 过采样倍数

BASE_DIR = '/workspace/Optical_ChestXRay'
DATA_PATH = os.path.join(BASE_DIR, 'data')
LOG_FILE = os.path.join(BASE_DIR, 'output', 'optical_v8_report.txt')
...
def init_log():
    with open(LOG_FILE, "w", encoding="utf-8") as f:
        f.write("===== 光计算加速医学影像诊断 V8（针对性优化）=====\n")
        f.write(f"时间：{time.strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write("改进：FocalLoss+FC精简+验证集分层(NORMAL 45-50%)+NORMAL过采样5x+TTA10\n\n")


label_smoothing = 0.05      # V8: 标签平滑+Focal组合
TEMPERATURE = 1.5           # V8: 温度校准
...
class OpticalChestXRayV8(nn.Module):
    ...
    def __init__(self, in_channels=1, num_classes=2, dropout_rate=0.5):
        ...
        # ====== V8：精简三级全连接 ======
        self.fc0 = nn.Linear(128 * 8 * 8, 384)        # 8192 -> 256
        self.bn0 = nn.BatchNorm1d(384)
        self.drop0 = nn.Dropout(dropout_rate)

        self.fc1 = nn.Linear(384, 64)                 # 256 -> 64
...
def main():
    ...
    log_print(f"V8优化: FC(8192->384->64->2) | Dropout={dropout_rate} | WD={weight_decay}")
    log_print(f"FocalLoss(gamma={focal_gamma}) | NORMAL过采样{NORMAL_OVERSAMPLE}x | TTA15+温度校准(T=1.5) | 验证集分层(NORMAL 45-50%)")
    ...
    # ====== V8：验证集分层(NORMAL 45-50%)采样（保证NORMAL占比≥35%）======
    # 获取每个类别的索引
