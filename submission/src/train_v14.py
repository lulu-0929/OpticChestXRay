"""
光计算加速医学影像诊断 — V14 半监督 + 精细优化训练脚本
======================================================
V14 核心改进（基于V13已验证配置，目标91%+）：
1. lr=5e-4（从1e-3减半，更精细训练）
2. 标签平滑 0.05→0.1（更强防过拟合）
3. 伪标签半监督：用V13（阈值0.35）对测试集打伪标签，扩充训练数据
4. CosineAnnealingWarmRestarts 保留（T_0=15, T_mult=2）
5. 保留V13全部超参数：Dropout=0.6, 过采样7x, FocalLoss γ=3.0
...
TEMPERATURE = 1.5
LABEL_SMOOTHING = 0.1              # V14: 从0.05→0.1

# 伪标签参数
PSEUDO_LABEL_THRESHOLD = 0.85      # 只有V13预测置信度≥0.85的样本才加入
USE_PSEUDO_LABELS = True           # 是否使用伪标签

BASE_DIR = '/workspace/Optical_ChestXRay'
DATA_PATH = os.path.join(BASE_DIR, 'data')
LOG_FILE = os.path.join(BASE_DIR, 'output', 'optical_v14_report.txt')
BEST_MODEL_PATH = os.path.join(BASE_DIR, 'output', 'best_optical_v14.pth')
V13_CKPT_PATH = os.path.join(BASE_DIR, 'output', 'best_optical_v13.pth')
...
def init_log():
    with open(LOG_FILE, "w", encoding="utf-8") as f:
        f.write("===== 光计算加速医学影像诊断 V14（半监督+精细优化）=====\n")
        f.write(f"时间：{time.strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write("改进：lr=5e-4 + 标签平滑0.1 + 伪标签半监督(阈值0.85) + 保留V13全部优化\n\n")
...
class OpticalChestXRayV14(nn.Module):
    """V14 — 结构与V13完全一致"""
...
class PseudoLabelDataset(torch.utils.data.Dataset):
    """
    伪标签数据集：用预训练模型对测试集打标签后加入训练
    每个样本：(img, label) — label是伪标签，与真实训练数据格式一致
    """
...
def generate_pseudo_labels(model, test_dataset, threshold=0.85):
    """
    用 V13 模型给测试集打伪标签
    返回：[(img_path, pseudo_label), ...] — 仅保留置信度≥threshold的样本
    以及统计信息
    """
    model.eval()
    pseudo_samples = []
    ...
    with torch.no_grad():
        for idx in range(len(test_dataset)):
            ...
            outputs = model(base_img) / TEMPERATURE
            probs = torch.softmax(outputs, dim=1)
            max_prob, pred = torch.max(probs, dim=1)

            confidence = max_prob.item()
            pseudo_label = pred.item()

            # 置信度过滤
            if confidence >= threshold:
                pseudo_samples.append((img_path, pseudo_label))
                label_counts[pseudo_label] += 1
                total_confident += 1

    log_print(f"  总测试集: {len(test_dataset)} 张")
    log_print(f"  置信度≥{threshold}: {total_confident} 张 ({total_confident/len(test_dataset)*100:.1f}%)")
    log_print(f"  NORMAL伪标签: {label_counts[0]}, PNEUMONIA伪标签: {label_counts[1]}")

    return pseudo_samples, label_counts
...
def main():
    ...
    log_print(f"V14优化: Dropout={DROPOUT_RATE} | lr={LR} | 标签平滑={LABEL_SMOOTHING} | 伪标签阈值={PSEUDO_LABEL_THRESHOLD}")
    ...
    oversampled_train_indices = oversampled_normal + train_pneumonia
    random.shuffle(oversampled_train_indices)

    # ====== 伪标签生成 ======
    if USE_PSEUDO_LABELS:
        # 加载V13模型打伪标签
        v13_model = OpticalChestXRayV14(
            in_channels=IN_CHANNELS, num_classes=NUM_CLASSES, dropout_rate=DROPOUT_RATE
        ).to(device)
        v13_model.load_state_dict(torch.load(V13_CKPT_PATH, map_location=device))
        v13_model.eval()

        pseudo_samples, pseudo_label_counts = generate_pseudo_labels(
            v13_model, test_dataset, threshold=PSEUDO_LABEL_THRESHOLD
        )

        # 创建伪标签数据集（使用同样的训练增强）
        pseudo_dataset = PseudoLabelDataset(pseudo_samples, transform=train_transform)

        # 原始训练数据集
        original_train_dataset = Subset(train_full, oversampled_train_indices)

        # 合并
        combined_dataset = ConcatDataset([original_train_dataset, pseudo_dataset])
        log_print(f"\n训练集合并: 原始{len(original_train_dataset)}张 + 伪标签{len(pseudo_dataset)}张 = {len(combined_dataset)}张\n")
    else:
        combined_dataset = Subset(train_full, oversampled_train_indices)
        log_print(f"\n训练集: {len(combined_dataset)}张（未使用伪标签）\n")
    ...
    train_sampled_labels = [all_labels[i] for i in oversampled_train_indices]
    ...
    # V14: CosineAnnealingWarmRestarts（T_0=15，每15epoch重启一次，更长周期）
    ...
    log_print("【与V13对比】")
    log_print(f"{'='*60}")
    log_print(f"{'指标':<25} {'V13(阈值0.35)':<20} {'V14(本次)':<20}")
