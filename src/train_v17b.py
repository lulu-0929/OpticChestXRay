"""
光计算加速医学影像诊断 — V17B 集成模型B（seed=100, FC 512→64）
===========================================================
基于V13最优配置，仅修改：
1. seed=100（与V13的seed=42不同，保证多样性）
2. FC 8192→512→64→2（V13是8192→384→64→2）
其余与V13完全一致
...
TRAIN_SEED = 100                # 与V13不同（V13=42）

BASE_DIR = '/workspace/Optical_ChestXRay'
DATA_PATH = os.path.join(BASE_DIR, 'data')
LOG_FILE = os.path.join(BASE_DIR, 'output', 'optical_v17b_report.txt')
BEST_MODEL_PATH = os.path.join(BASE_DIR, 'output', 'best_optical_v17b.pth')
...
def init_log():
    with open(LOG_FILE, "w", encoding="utf-8") as f:
        f.write("===== 光计算加速医学影像诊断 V17B（集成模型B）=====\n")
        f.write(f"时间：{time.strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"seed={TRAIN_SEED} | FC 8192→512→64→2 | Dropout={dropout_rate}\n\n")
...
class OpticalChestXRayV17B(nn.Module):
    """V17B：FC 8192→512→64→2（V13是8192→384→64→2）"""
...
def main():
    ...
    log_print(f"V17B: seed={TRAIN_SEED} | FC 8192→512→64→2 | Dropout={dropout_rate}")
    log_print(f"FocalLoss(γ={focal_gamma}) + LabelSmoothing={label_smoothing}")
    ...
    electrical_ops = 16*64*64 + 32*32*32 + 64*16*16 + 128*8*8 + 512 + 64
