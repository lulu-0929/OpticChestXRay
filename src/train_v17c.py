"""
光计算加速医学影像诊断 — V17C 集成模型C（seed=200, Dropout=0.5）
===========================================================
基于V13最优配置，仅修改：
1. seed=200（与V13=42, V17B=100均不同，最大多样性）
2. Dropout=0.5（从0.6降回0.5，探索不同正则化强度）
3. 其余与V13完全一致（8192→384→64→2）
...
lr = 1e-3
weight_decay = 5e-4
dropout_rate = 0.5              # V17C: 从0.6降回0.5
focal_gamma = 3.0
TTA_NUM = 15
NORMAL_OVERSAMPLE = 7
TEMPERATURE = 1.5
label_smoothing = 0.05
TRAIN_SEED = 200                # 与V13(42), V17B(100)均不同

BASE_DIR = '/workspace/Optical_ChestXRay'
DATA_PATH = os.path.join(BASE_DIR, 'data')
LOG_FILE = os.path.join(BASE_DIR, 'output', 'optical_v17c_report.txt')
BEST_MODEL_PATH = os.path.join(BASE_DIR, 'output', 'best_optical_v17c.pth')
...
def init_log():
    with open(LOG_FILE, "w", encoding="utf-8") as f:
        f.write("===== 光计算加速医学影像诊断 V17C（集成模型C）=====\n")
        f.write(f"时间：{time.strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"seed={TRAIN_SEED} | Dropout=0.5 | 其余与V13一致\n\n")
...
class OpticalChestXRayV17C(nn.Module):
    """V17C：与V13架构一致（8192→384→64→2），仅seed和dropout不同"""
...
def main():
    ...
    log_print(f"V17C: seed={TRAIN_SEED} | Dropout={dropout_rate} | 与V13同架构")
